...
def init_finalize_routes(app, DB_PATH):
    ...
    def get_finalize_status_v2():
        ...
        try:
            ...
            if not exam: return jsonify({"success": True, "no_exam": True, "message": "No Active Examination Found", "icon": "📅"})
            ex_id = exam["id"]
            ex_id = exam['id']
            
            # Check if result is already locked for this class
            ...
            subjects = conn.execute("""
                ...
                (SELECT COUNT(*) FROM student_marks WHERE exam_id=? AND subject_id=s.id 
            ...
            status_list = [{"id": r['id'], "subject": r['name'], "teacher": r['teacher'], "submitted": r['entries'] > 0} for r in subjects]
            ...
            if all_ready and not is_published:
                ...
                for st in st_rows:
                    stats = conn.execute('SELECT SUM(obtained_marks) as obt, SUM(total_marks) as tot FROM student_marks WHERE exam_id=? AND student_id=? AND subject_id > 0', (ex_id, st['id'])).fetchone()
                    obt, tot = (stats['obt'] or 0), (stats['tot'] or 0)
                    perc = round((obt/tot*100), 1) if tot > 0 else 0
                    students_data.append({"id": st['id'], "name": st['full_name'], "father_name": st['father_name'], "perc": perc})
            
            return jsonify({"success": True, "exam_name": exam['name'], "exam_id": ex_id, "is_published": is_published, "status": status_list, "is_ready": all_ready, "students": students_data})
        except Exception as e: return jsonify({"success": False, "error": str(e)})
        finally: conn.close()

    @app.route('/api/get-student-report-card')
    def get_student_report_card():
        ex_id = request.args.get('exam_id'); st_id = request.args.get('student_id'); u = session.get('user')
        conn = sqlite3.connect(DB_PATH); conn.row_factory = sqlite3.Row
        try:
            marks = conn.execute("""
    ...
    def get_subject_marks_details():
        ex_id = request.args.get('exam_id'); sub_id = request.args.get('subject_id'); u = session.get('user')
        conn = sqlite3.connect(DB_PATH); conn.row_factory = sqlite3.Row
        try:
            marks = conn.execute("""
                SELECT st.full_name, m.obtained_marks, m.total_marks 
            ...
            marks_list = [{"name": r['full_name'], "obt": r['obtained_marks'], "tot": r['total_marks'], "is_pass": float(r['obtained_marks'] or 0) >= (float(r['total_marks'] or 1) * 0.33)} for r in marks]
            p = sum(1 for m in marks_list if m['is_pass']); f = len(marks_list) - p
            return jsonify({"success": True, "marks": marks_list, "stats": {"total": len(marks_list), "pass": p, "fail": f}})
        except Exception as e: return jsonify({"success": False, "error": str(e)})
        finally: conn.close()
    ...
    def publish_final():
        ...
        try:
            ...
            for s_id, rmk in remarks.items():
                # Insert Final Grade Entry (subject_id=0)
                ...
                # Lock all subject marks for this student
                conn.execute("UPDATE student_marks SET is_locked=1 WHERE exam_id=? AND student_id=?", (ex_id, s_id))
            conn.commit()
            return jsonify({"success": True, "message": "Result Locked Successfully!"})
