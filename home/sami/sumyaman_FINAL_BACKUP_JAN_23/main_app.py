...
def check_attendance_state(t_class, t_sec, t_wing):
    ...
    cursor.execute("""SELECT MAX(edit_count) FROM apsokara_attendance 
                      ...
                      AND student_id IN (SELECT identity_number FROM apsokara_student WHERE wing=?)""", 
...
def fetch_user_data(user_id, dob_val, role):
    ...
    try:
        if role == "Student":
            q = "SELECT full_name, father_name, identity_number, dob, wing, student_class, student_section, roll_no FROM apsokara_student WHERE identity_number=? AND dob=?"
            ...
            q = "SELECT full_name, father_name, identity_number, dob, wing, incharge_class, incharge_section, is_class_incharge FROM apsokara_teacher WHERE identity_number=? AND dob=?"
            cursor.execute(q, (user_id, dob_val))
            row = cursor.fetchone()
            if row:
